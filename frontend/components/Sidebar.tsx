...
const ICONS: Record<string, React.ReactNode> = {
  ...
  plus: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 5v14m-7-7h14"/></svg>,
...
const NAV_ITEMS: Record<string, { key: string; label: string; icon: string }[]> = {
  EMPLOYEE: [
  ...
  MANAGER: [
    ...
    { key: "dash",    label: "My Dashboard",   icon: "grid" },
    { key: "submit",  label: "New Expense",    icon: "plus" },
    { key: "history", label: "My Expenses",    icon: "file" },
  ],
  ADMIN: [
...
interface Props { activeTab: string; onTabChange: (key: string) => void; pendingCount?: number; }
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sb-footer">
        ...
        <button
          onClick={logout}
          style={{ fontSize: "0.72rem", color: "var(--text-muted)", background: "none", border: "none", cursor: "pointer", padding: 0, marginTop: "6px" }}
        >
          Sign out →
        </button>
      </div>
