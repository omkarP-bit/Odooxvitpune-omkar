    ...
    <div className="fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">Company-wide expense ledger — {filtered.length} total</p>
        </div>
      </div>

      <div className="searchbar">
        <input className="search-input" placeholder="Search employee, description, vendor..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
      ...
      <div className="card">
        <table className="tbl">
          <thead>
            <tr>
              <th>Employee</th><th>Description</th><th>Category</th><th>Original</th><th>Converted</th><th>Date</th><th>Status</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="text-center" style={{ padding: "24px", color: "var(--text-muted)" }}>No expenses found</td></tr>
            ) : paginated.map((e) => (
              <tr key={e.publicId}>
                <td style={{ color: "var(--text)", fontWeight: 600 }}>{e.user?.name ?? "—"}</td>
